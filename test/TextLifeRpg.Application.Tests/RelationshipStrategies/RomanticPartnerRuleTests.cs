...
public class RomanticPartnerRuleTests
{
  #region Fields
  ...
  public void Generate_ShouldNotExceedMaxPairs()
  {
    ...
    A.CallTo(() => _relationshipFactory.Create(
        A<List<Relationship>>._, A<Character>._, A<Character>._, RelationshipType.RomanticPartner, now
      )
    ).ReturnsLazily(call =>
      {
        ...
        return
        [
          Relationship.Create(x.Id, y.Id, RelationshipType.RomanticPartner, now, now, 90),
          Relationship.Create(y.Id, x.Id, RelationshipType.RomanticPartner, now, now, 90)
        ];
      }
    ).NumberOfTimes(1);

    var rule = new RomanticPartnerRule(
  ...
  public void Generate_ShouldCreateReciprocalRomanticRelationships()
  {
    ...
    A.CallTo(() => _randomProvider.Next(A<int>._, A<int>._)).Returns(0);
    var relA = Relationship.Create(a.Id, b.Id, RelationshipType.RomanticPartner, now, now, 100);
    var relB = Relationship.Create(b.Id, a.Id, RelationshipType.RomanticPartner, now, now, 100);

    A.CallTo(() => _relationshipFactory.Create(A<List<Relationship>>._, a, b, RelationshipType.RomanticPartner, now))
      .Returns([relA, relB]);
    ...
    Assert.Equal(RelationshipType.RomanticPartner, relAb.Type);

    Assert.Equal(a.Id, relBa.TargetCharacterId);
    Assert.Equal(RelationshipType.RomanticPartner, relBa.Type);
  }
  ...
  [Fact]
  public void Generate_ShouldSkipSubsequentPairs_WhenCharacterEntersRomanticRelationshipEarlierInSameRun()
  {
    // Arrange
    var now = new DateOnly(2025, 6, 16);
    var chars = Enumerable.Range(0, 16).Select(_ => new CharacterBuilder().Build()).ToList();
